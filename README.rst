...
Disclaimer: The original manual has been slightly altered to depict the actual file formats, extensions, blast commands and step by step instructions.
MCScanX
=========
...
Here is a typical parameter setting for generating the xyz.blast file::
	...
	In case you are using recent blast programs e.g; 2.9 or above instead try the following commandline:
	
	make blast db
	:::::::::::::
	
	$makeblastdb -in pcc9339.faa -dbtype prot -out pcc9339.db -parse_seqids
	
	Run Blastp
	::::::::::
	
	$blastp -db pcc9339.db -query 74FromGenbank.faa  -out 74.out  -evalue 1e-10 -outfmt 6 -num_alignments 5 -num_descriptions 5
...
- Output
The execution of MCScanX outputs one text file xyz.collinearity, containing pairwise syteny blocks as follows::
